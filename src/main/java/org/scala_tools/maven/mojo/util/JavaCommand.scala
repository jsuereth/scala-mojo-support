...
class JavaCommand(requester: AbstractMojo, mainClassName: String, classpath: String, var jvmArgs: Seq[String], var args: Seq[String]) {
  var logOnly = false;
  val env = for(key <- System.getenv().keySet) yield key + "=" + System.getenv(key)
  val javaHome = {
    val tmp = System.getProperty("java.home")
    if(tmp == null) System.getProperty("JAVA_HOME")
    else tmp
  }
  if(javaHome == null) {
  ...
  jvmArgs = "-classpath" +: classpath +: jvmArgs 
  
  /** Builds the sequence used to create a process builder */
  private def buildCommand = javaExec +: (jvmArgs.toList ++ (mainClassName +: args))
  
  //Methods
  @throws(classOf[Exception])
  def run(displayCmd: Boolean, throwFailure: Boolean): Unit = {
