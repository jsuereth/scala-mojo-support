...
class ScalaComponentFactory extends AbstractComponentFactory {
  ...
  override def newInstance(componentDescriptor: ComponentDescriptor[_], 
    ...
    def createScalaObject(constructor: java.lang.reflect.Constructor[_]) = 
    ...
    try {
      ...
      val constructorArgs : Array[PlexusConfiguration] = 
        if(componentDescriptor.getConfiguration != null)
	       (for {        
      ...
      else createScalaObject(constructor)
    } catch {
      case t : Throwable =>
