...
class ScalaConfigurationConverter extends AbstractConfigurationConverter {
  ...
  override def canConvert(someType: Class[_]): Boolean = true
...
class ScalaVarSetter(fieldName: String, obj: AnyRef, lookup: ConverterLookup, listener: ConfigurationListener) {
  ...
  def configure(config: PlexusConfiguration, cl: ClassLoader, evaluator: ExpressionEvaluator ): Unit = {
    val value = setterTypeConverter.fromConfiguration(lookup, config, setterParamType, obj.getClass(), cl,
	                                                  evaluator, listener );
    if ( value != null ) setValueUsingSetter(value)
  } 
 
  private def setValueUsingSetter(value: AnyRef): Unit = {
     if(setterParamType == null) 
       throw new ComponentConfigurationException( "No setter found" )
     val exceptionInfo = 
       obj.getClass().getName() + "." + fieldName + " = ( " + setterParamType.getClass().getName() + " )"
       
     if ( listener != null ) listener.notifyFieldChangeUsingSetter( fieldName, value, obj )
     try {
       import converters.ReflectionUtil._
